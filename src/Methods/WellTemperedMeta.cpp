...
{
	// Pre-simulation hook.
	void WellTemperedMeta::PreSimulation(Snapshot* snapshot, const CVManager& cvmanager)
	{
		auto cvs = cvmanager.GetCVs(cvmask_);
		// Write ouput file header.
    std::cout << "potential_iinitial:  " << potential_initial_ << std::endl;
    ...
    // Initialize grid_potential by adding up hills.
      ...
      for (auto& hill: hills_) {
        ...
        for(; it != grid_->end(); ++it, ++ip)
        {
            ...
            // Add to grid.
            for(size_t i = 0; i < n; ++i){
                val[i] += height_ * exp(-(*grid_potential_)[hill.center] / delta_T_) * df[i];
            }
            // Add to grid_potential.
            valp += height_ * exp(-(*grid_potential_)[hill.center] / delta_T_) * dp;
        }
      }
    }
	...
	// Drop a new hill.
	void WellTemperedMeta::AddHill(const CVList& cvs, int iteration)
	{
		int n = cvs.size();
      ...
      //determine which grid is cval on
			hills_.emplace_back(cval, widths_, height_ * exp(-(*grid_potential_)[cval] / delta_T_));
      ...
      dp = 1.0;
        ...
        dp *= gaussian(dxp[i], hill.width[i]);
          ...
          // Need to reduce the dimension of grid_potential by size of cvs.
	 		...
	 		// Add to grid. 
	 		for(size_t i = 0; i < n; ++i){
        val[i] += height_ * exp(-(*grid_potential_)[cval] / delta_T_) * df[i];
      }
     // Add to grid_potential.
      valp += height_ * exp(-(*grid_potential_)[cval] / delta_T_) * dp;
	 	}
	}


	void WellTemperedMeta::CalcBiasForce(const CVList& cvs)
	{	
